...
public class SecurityConfig {
    ...
    SecretKeySpec key(@Value("${app.jwt-secret}") String secret) {
        if (secret.getBytes(StandardCharsets.UTF_8).length < 32) {
            throw new IllegalArgumentException("JWT_SECRET precisa de 32 bytes ou mais");
        }
    ...
    @Bean
    JwtDecoder decoder(SecretKeySpec key, @Value("${app.jwt-issuer}") String issuer) {
    ...
    @Bean
    SecurityFilterChain filter(HttpSecurity http, JwtAuthenticationConverter converter) throws Exception {
